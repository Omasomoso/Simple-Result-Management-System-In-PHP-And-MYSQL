...
// Redirect if session/term/class not selected
if (!$session_id || !$term_id || !$class_id) {
    header("Location: admin_select_session.php"); // Assuming this is a placeholder or needs to be created
    exit();
}
...
// Fetch all students in the selected class with their class names
$stmt_students = $pdo->prepare("SELECT students.*, classes.name AS class_name 
                        FROM students 
...
// Fetch session and term names once
$stmt_session = $pdo->prepare("SELECT name FROM academic_sessions WHERE id = ?");
$stmt_session->execute([$session_id]);
$session = $stmt_session->fetchColumn();

$stmt_term = $pdo->prepare("SELECT name FROM terms WHERE id = ?");
$stmt_term->execute([$term_id]);
$term = $stmt_term->fetchColumn();
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .table, .skills-table, .signature-line {
...
<body>
    <div class="admin-controls no-print">
        <h1>Printing All Students Results</h1>
        <p>Class: <?= htmlspecialchars($class_name) ?> | Session: <?= htmlspecialchars($session) ?> | Term: <?= htmlspecialchars($term) ?></p>
    ...
    $stmt_results = $pdo->prepare("
        ...
        ORDER BY s.name
    ");
    $stmt_results->execute([$student['id'], $session_id, $term_id]);
    $results = $stmt_results->fetchAll();
    
    $stmt_psychomotor = $pdo->prepare("SELECT * FROM psychomotor_skills WHERE student_id = ? AND session_id = ? AND term_id = ?");
    $stmt_psychomotor->execute([$student['id'], $session_id, $term_id]);
    $psychomotor = $stmt_psychomotor->fetch();

    $stmt_affective = $pdo->prepare("SELECT * FROM affective_skills WHERE student_id = ? AND session_id = ? AND term_id = ?");
    $stmt_affective->execute([$student['id'], $session_id, $term_id]);
    $affective = $stmt_affective->fetch();

    $stmt_gpa = $pdo->prepare("SELECT gpa, cgpa FROM gpa_records WHERE student_id = ? AND session_id = ? AND term_id = ?");
    $stmt_gpa->execute([$student['id'], $session_id, $term_id]);
    $gpa = $stmt_gpa->fetch();
    ?>
    <div class="container student-result">
        <!-- School Header -->
        ...
        <?php if (empty($results)): ?>
            <div class="no-results">
                No results with valid scores found for this student.
            </div>
        <?php else: ?>
            <table class="table">
                <thead>
